...
//  console.log(key);
  request(key,{json:true}, (err,res,body) =>{
    for(i in body.results){
      var bod=body.results[i];
      if(bod.components._type == "city" || bod.components._type== "state" || bod.components._type=="county"){
        if((bod.components.city.toUpperCase()== city.toUpperCase() || bod.components.state.toUpperCase()==city.toUpperCase() || bod.components.county.toUpperCase() ==city.toUpperCase()) && bod.components.country.toUpperCase()== country.toUpperCase()){
          callback(bod.geometry.lat+" "+bod.geometry.lng);
          break;
        }
      }
